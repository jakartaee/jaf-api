...
MimeTypeRegistryProvider must be implemented when the application wishes
...
Application can use its own CommandMap implementation or use the
implementation that is included in Jakarta Activation (MailcapCommandMap).
When the second option is used MimeTypeRegistry, MailcapRegistry,
MailcapRegistryProvider and MimeTypeRegistryProvider must be
implemented.
...
and retrieve MailcapEntries. Application must implement
...
to store and retrieve MimeTypeEntries. Application must implement
